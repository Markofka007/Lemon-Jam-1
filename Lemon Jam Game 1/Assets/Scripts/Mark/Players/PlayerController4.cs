...
public class PlayerController4 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        fist = transform.GetChild(0).GetComponent<FistAttack4>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision.CompareTag("PowerBean"))
        {
            ...
            {
                equipedItem.GetComponent<AutoGun4>().MultiplyPower(powerM_Delta);
            }
            else if (equipedItem.name.Contains("Bellow"))
            {
                equipedItem.GetComponent<Bellow4>().MultiplyPower(powerM_Delta);
            }
            else if (equipedItem.name.Contains("RocketLauncher"))
            {
                equipedItem.GetComponent<RL4>().MultiplyPower(powerM_Delta);
            }
            else if (equipedItem.name.Contains("Bat"))
            {
                equipedItem.GetComponent<Bat4>().MultiplyPower(powerM_Delta);
            }
            else if (equipedItem.name.Contains("LaserCannon"))
            {
                equipedItem.GetComponent<LC4>().MultiplyPower(powerM_Delta);
            }
        }
    }
    ...
    public void Fire(InputAction.CallbackContext context)
    {
        if (context.performed)
        {
            ...
            {
                equipedItem.GetComponent<RL4>().Fire();
            }
            else if (equipedItem.name.Contains("Bat"))
            {
                equipedItem.GetComponent<Bat4>().Fire();
            }
        }
